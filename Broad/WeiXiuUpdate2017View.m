...
@interface WeiXiuUpdate2017View ()<UICollectionViewDataSource, UICollectionViewDelegate,UITextFieldDelegate,HSDatePickerViewControllerDelegate,UIAlertViewDelegate,UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
{
    MatnRec *newmatnRec;
...
@implementation WeiXiuUpdate2017View
...
-(void) bindData
{
    NSString *machineOther = newmatnRec.MachineOther;
    ...
    else if ([newmatnRec.Project isEqualToString:@"年3次保养"])
    {
        if(isJieDian)
        {
            ...
            self.scrollView.contentSize = CGSizeMake(self.scrollView.frame.size.width, self.imgContain_view.frame.origin.y + self.img3_view.frame.origin.y + self.img3_view.frame.size.height);
    ...
    else if ([newmatnRec.Project isEqualToString:@"年4次保养"])
    {
        if(isJieDian)
        {
            ...
            self.scrollView.contentSize = CGSizeMake(self.view.frame.size.width, self.imgContain_view.frame.origin.y + self.img4_view.frame.origin.y + self.img4_view.frame.size.height);
        ...
        else
        {
            ...
            self.scrollView.contentSize = CGSizeMake(self.view.frame.size.width, self.imgContain_view.frame.origin.y + self.img7_view.frame.origin.y + self.img7_view.frame.size.height);
    ...
    if([self.servcetype_field.text isEqualToString:@"年4次保养"])
    {
        for(NSString *key in imgDic)
        {
            ...
            if(img)
            {
                ...
                NSString *reName = [NSString stringWithFormat:@"%@%@%i%@.jpg",project,[Tool getCurrentTimeStr:@"yyyy-MM-dd-HHmmss"],y, @"I"];
...
- (void)hsDatePickerPickedDate:(NSDate *)date
{
    ...
    long year = [[Tool transformDateFormat:sysTimeStr andFromFormatterStr:@"yyyy-MM-dd HH:mm" andToFormatterStr:@"yyyy"] longLongValue];
    
    //2016年底修改为:服务时间小于等于当前时间
    NSString *now = [Tool transformDateFormat:sysTimeStr andFromFormatterStr:@"yyyy-MM-dd HH:mm" andToFormatterStr:@"yyyy-MM-dd"];
        ...
        if(tag == 0 || tag == 1)
        {
            ...
            if(tag == 0 || tag == -1)
            {
                if(selectTimeIndex == 4)
                {
                    ...
                    {
                        [Tool showCustomHUD:@"服务时间不能超过当前时间" andView:self.view andImage:nil andAfterDelay:1.2f];
                        return;
                    }
                }
                 ...
                 [[AFOSCClient  sharedClient] getPath:[NSString stringWithFormat:@"%@GetNowDateTime",api_base_url] parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject)
                  {
                      ...
                      utils.parserOK = ^(NSString *string)
                      {
                          hud.hidden = YES;
                          sysTimeStr = string;
                          NSString *sysTimeStryyyymmdd = [string substringToIndex:[string rangeOfString:@" "].location];
                          currentMonthDayStr = [Tool transformDateFormat:sysTimeStryyyymmdd andFromFormatterStr:@"yyyy-MM-dd" andToFormatterStr:@"MMdd"];
                          self.uploadtime_label.text = sysTimeStr;
                      };
