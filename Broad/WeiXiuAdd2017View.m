...
@interface WeiXiuAdd2017View ()<UICollectionViewDataSource, UICollectionViewDelegate,UITextFieldDelegate,HSDatePickerViewControllerDelegate,UIAlertViewDelegate,UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
{
    MatnRec *matnRec;
...
@implementation WeiXiuAdd2017View
...
- (void)add
{
    ...
    matnRec.allfilename10 = @"";
    matnRec.allfilename11 = @"";
    matnRec.allfilename12 = @"";
    
    long year = [[Tool transformDateFormat:self.uploadtime_label.text andFromFormatterStr:@"yyyy-MM-dd HH:mm" andToFormatterStr:@"yyyy"] longLongValue];
    ...
    if ([self.servcetype_field.text isEqualToString:@"年4次保养"])
    {
        ...
        if ([self.serviceproject_field.text isEqualToString:@"年1次保养"])
        {
            ...
            else if (currentMonthDayLong <= 325)
            {
    ...
    if([self.servcetype_field.text isEqualToString:@"年4次保养"])
    {
        for(NSString *key in imgDic)
        {
            ...
            if(imgbegin)
            {
                ...
                reName = nil;
                reName = [NSString stringWithFormat:@"%@%@%i%@.jpg",project,[Tool getCurrentTimeStr:@"yyyy-MM-dd-HHmmss"],y, @"I"];
                        ...
                        if ([self.servcetype_field.text isEqualToString:@"年4次保养"])
                        {
                            switch (index)
                            {
                                ...
                                case 9:
                                    ...
                                    break;
                                case 10:
                                    matnRec.allfilename10 = [NSString stringWithFormat:@"|%@",fileName];
                                    break;
                                case 11:
                                    matnRec.allfilename11 = [NSString stringWithFormat:@"|%@",fileName];
                                    break;
                                case 12:
                                    matnRec.allfilename12 = [NSString stringWithFormat:@"|%@",fileName];
                                    break;
                            }
                        }
                        else
...
- (void)writeLog
{
    ...
    [request setUseCookiePersistence:NO];
    [request setTimeOutSeconds:30];
    NSString *sql = [NSString stringWithFormat:@"exec sp_executesql N'insert into [ERPRiZhi] (UserName,Operation,Plate,ProjName,DoSomething,IpStr) values (@UserName,@Operation,@Plate,@ProjName,@DoSomething,@IpStr);select @@IDENTITY',N'@UserName varchar(50),@Operation varchar(20),@Plate varchar(100),@ProjName varchar(500),@DoSomething varchar(1000),@IpStr varchar(50)',@UserName='%@',@Operation='新增',@Plate='维护保养(%@)',@ProjName='%@',@DoSomething='工程师新增(中文app:IOS);机组编号:%@',@IpStr='%@'", UserName, self.serviceproject_field.text, depart.PROJ_Name, self.engine_no_label.text, ip];
...
- (void)enginChoice
{
    [SGActionView showSheetWithTitle:@"请选择机组:"
                      ...
                      selectedHandle:^(NSInteger index){
                          ...
                          NSString *machineOther = unit.MachineOther;
                          if([self.servcetype_field.text isEqualToString:@"年4次保养"])
                          {
...
//                                      self.img12_view.hidden = YES;
//                                      
//                                      self.scrollView.contentSize = CGSizeMake(self.scrollView.frame.size.width, self.img4_view.frame.origin.y + self.img4_view.frame.size.height + 554);
//                                      
//                                      CGRect viewFrame = self.imgContain_view.frame;
//                                      viewFrame.size.height = self.img4_view.frame.origin.y + self.img4_view.frame.size.height;
//                                      self.imgContain_view.frame = viewFrame;
...
//                                      self.img12_view.hidden = YES;
//                                      
//                                      self.scrollView.contentSize = CGSizeMake(self.view.frame.size.width, self.img7_view.frame.origin.y + self.img7_view.frame.size.height + 554);
//                                      
//                                      CGRect viewFrame = self.imgContain_view.frame;
...
- (void)initNianFour
{
    self.img1_view.hidden = NO;
    self.img2_view.hidden = NO;
    self.img3_view.hidden = NO;
    self.img4_view.hidden = NO;
    self.img5_view.hidden = NO;
    self.img6_view.hidden = NO;
    self.img7_view.hidden = NO;
    self.img8_view.hidden = NO;
    self.img9_view.hidden = NO;
    self.img10_view.hidden = NO;
    self.img11_view.hidden = NO;
    self.img12_view.hidden = NO;
    long currentMonthDayInt = [currentMonthDayStr longLongValue];
    //年1次保养
    if (currentMonthDayInt >= 1226 || currentMonthDayInt <= 325)
    {
        ...
        if(isJieDian)
        {
            ...
            self.scrollView.contentSize = CGSizeMake(self.view.frame.size.width, self.img2_view.frame.origin.y + self.img2_view.frame.size.height + 554 );
            CGRect viewFrame = self.imgContain_view.frame;
            viewFrame.size.height = self.img2_view.frame.origin.y + self.img2_view.frame.size.height;
        }
        else
        {
            self.img1_label.text = @"主体真空值";
    ...
    else if (currentMonthDayInt >= 626 && currentMonthDayInt <= 925)
    {
        ...
        if(isJieDian)
        {
            ...
            self.scrollView.contentSize = CGSizeMake(self.view.frame.size.width, self.img3_view.frame.origin.y + self.img3_view.frame.size.height + 554);
            
            CGRect viewFrame = self.imgContain_view.frame;
            viewFrame.size.height = self.img3_view.frame.origin.y + self.img3_view.frame.size.height;
            self.imgContain_view.frame = viewFrame;
        }
        else
        {
...
- (void)hsDatePickerPickedDate:(NSDate *)date
{
    ...
    long year = [[Tool transformDateFormat:sysTimeStr andFromFormatterStr:@"yyyy-MM-dd" andToFormatterStr:@"yyyy"] longLongValue];
    if ([self.servcetype_field.text isEqualToString:@"年4次保养"])
    {
        NSString *start = nil;
        NSString *end = nil;
        ...
        int tag = [Tool compareOneDay:targetDate withAnotherDay:sysTimeStr];
        if(tag == 0 || tag == -1)
        {
                 ...
                 [[AFOSCClient  sharedClient] getPath:[NSString stringWithFormat:@"%@GetNowDateTime",api_base_url] parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject)
                  {
                      ...
                      utils.parserOK = ^(NSString *string)
                      {
                          ...
                          sysTimeStr = [string substringToIndex:[string rangeOfString:@" "].location];
                          currentMonthDayStr = [Tool transformDateFormat:sysTimeStr andFromFormatterStr:@"yyyy-MM-dd" andToFormatterStr:@"MMdd"];
